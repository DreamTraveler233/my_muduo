...
public:
    ...
    void setConnectionCallback(const ConnectionCallback &cb);      ///< 新连接建立回调
    void setMessageCallback(const MessageCallback &cb);            ///< 消息到达回调
    void setWriteCompleteCallback(const WriteCompleteCallback &cb);///< 数据发送完成回调
...
private:
    ...
    ThreadInitCallback threadInitCallback_;///< 线程池线程初始化回调
